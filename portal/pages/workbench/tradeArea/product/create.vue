  ...
  <div>
    <el-breadcrumb separator="/" class="bread">
      <el-breadcrumb-item :to="{ path: 'list' }">
        <span style="color:#666;font-weight: normal">产品管理</span>
      </el-breadcrumb-item>
      <el-breadcrumb-item>
        <span style="font-size: 14px; color: #101010">{{ id ? '编辑产品' : '发布产品' }}</span>
      </el-breadcrumb-item>
    </el-breadcrumb>
      ...
      <el-card class="form-card" shadow="never">
        <el-form :disabled="!isEditMode" style="width: 60%;" :model="demandForm" :rules="rules" ref="demandFormRef"
          ...
          <el-form-item label="产品分类:" prop="typeName">
            <el-cascader style="width: 300px" v-model="demandForm.typeName" :options="typeNameOptions"
              placeholder="请选择产品分类" />
          </el-form-item>

          <el-form-item label="产品名称:" prop="name">
            <el-input v-model="demandForm.name" placeholder="请输入产品名称" clearable></el-input>
          </el-form-item>

          <el-form-item label="图片" prop="image">
            <el-image v-if="demandForm.image" :src="demandForm.image" fit="cover"
              style="width: 100px;height: 100px;display: inline-block; margin-right: 15px;" />
            <el-upload :action="actionUrl" :show-file-list="false" :before-upload="(file) => {
          ...
          <el-form-item label="简介:" prop="description">
            <el-input type="textarea" :rows="3" v-model="demandForm.description" placeholder="请输入产品简介" />
          </el-form-item>

          <el-form-item label="价格:" prop="price">
            <el-input-number style="width: 300px" :precision="2" v-model="demandForm.price" :min="0"
              :max="9999999999.99" placeholder="请输入产品价格" />
          </el-form-item>
...
import { postPublish, postUnPublish, postSaveProduct, postUpdateProduct, getMyOrJoinedCircleList, getProductDetail } from '@/api/api'

const actionUrl = import.meta.env.VITE_BASE_URL + '/chain/upload'
...
const getDetail = async () => {
  ...
  demandForm.typeName = res.result.typeName
  demandForm.image = res.result.image
  demandForm.description = res.result.description
  ...
  demandFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        if (id.value) {
          await postUpdateProduct({
            ...demandForm,
            memberDID: cookies.getItem('did')
          })
          getDetail()
        } else {
          const res = await postSaveProduct({
            ...demandForm,
            memberDID: cookies.getItem('did')
          })
...
const handleUnPublish = async () => {
  ElMessageBox.confirm('撤销后圈子用户无法将查看该产品，确认撤销？', '撤销发布', {
