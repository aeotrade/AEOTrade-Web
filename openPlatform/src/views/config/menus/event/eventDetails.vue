<template>
  <div class="menu-container">
    <div class="title" v-if="!isConfig">
      ...
      / {{isDisabled?'编辑触发事件':'添加触发事件'}}
    </div>
    <div class="title" v-else>
      <span @click="isConfig = false">
        <i class="el-icon-arrow-left"></i>返回
      </span>
      / {{isDisabled?'编辑触发事件':'添加触发事件'}}/ 配置
    </div>
    <el-form v-if="!isConfig" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="名称：" prop="name">
        <el-input v-model.trim="ruleForm.name" placeholder="请输入触发事件名称"></el-input>
      </el-form-item>
      <el-form-item label="描述：" prop="desc">
        <el-input :rows="4" type="textarea" placeholder="请输入触发事件描述" v-model.trim="ruleForm.desc"></el-input>
      </el-form-item>
      <el-form-item label="编码：" prop="code">
        <el-input :disabled="isDisabled" v-model.trim="ruleForm.code" placeholder="编码为触发事件的唯一标识，支持大小写英文、下划线"></el-input>
      </el-form-item>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        rules: {
          name: [
            ...
            {min: 0, max: 32, message: '触发事件名称在32个字符内', trigger: 'blur'}
          ],
          desc: [
            {required: true, message: '请输入触发事件描述', trigger: 'blur'},
            {min: 0, max: 255, message: '触发事件描述长度在255个字符内', trigger: 'blur'}
          ],
          code: [
            {required: true, message: '请输入触发事件编码', trigger: 'blur'},
            {min: 0, max: 50, message: '触发事件编码长度在50个字符内', trigger: 'blur'}
          ],
          data_model: [
    ...
    methods: {
      getInfo(){
        this.$api.connector.infoEvent(this.$route.query.id,this.$route.query.ids).then(res=>{
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            if (this.$route.query.ids) {
              ...
              this.$api.connector.editEvent(this.$route.query.id, this.$route.query.ids,params).then(res => {
                if (res.code == 0) {
                  this.$message.success('编辑触发事件成功')
                  this.$router.go(-1)
                }
              })
            } else {
              this.$api.connector.creatEvent(this.$route.query.id, this.ruleForm).then(res => {
                if (res.code == 0) {
                  if(this.$store.state.user.connectorInfo.status === '未发布'){
                    this.$message.success('创建触发事件成功')
