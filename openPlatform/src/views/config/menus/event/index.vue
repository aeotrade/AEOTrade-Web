<template>
  <div>
    <div class="section">
      <div class="title">
        触发事件
        <span>触发事件是指当此动作发生时，应用同步数据并触发流程，例如：当有新数据添加时，或者有数据修改时。</span>
      </div>
      <div class="search-box">
        <span>名称：</span>
        <el-input v-model="search" placeholder="请输入触发事件名称" style="width: 280px;margin: 0 100px 0 16px"></el-input>
    ...
    <div class="section" style="height: calc(100vh - 220px)">
      <el-button size="small" type="primary" style="width: 120px;" @click="$router.push({path:'/config/eventDetails',query:$route.query})">添加触发事件</el-button>
      <el-table
        height="530px"
        :data="tableData"
        ...
        <div slot="empty" style="margin-bottom: 20px;">
          <div>
            <img src="@/assets/connector/noevent.svg" style="width: 230px;margin: 90px 0 0 0;">
          </div>
          <div>
            当前没有触发事件，请去<span class="tip" @click="$router.push({path:'/config/eventDetails',query:{id:$route.query.id}})">添加触发事件</span>！
          </div>
        </div>
      </el-table>
  ...
  export default {
    ...
    methods:{
      getList(search) {
        ...
        this.$api.connector.getEvent(this.$route.query.id,params).then(res => {
      ...
      handleDelete(row) {
        this.$confirm(this.$store.state.user.connectorInfo.status === '已发布'?'删除触发事件会影响到用户正在使用该事件的协作合约，你确认这样做吗？':'确认删除触发事件吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消'
        }).then(() => {
          this.$api.connector.delEvent(this.$route.query.id,row.id).then(res => {
      ...
      handleEdit(row) {
        this.$router.push({path:'/config/eventDetails',query:{id:this.$route.query.id,ids:row.id}})
      },
    }
  }
</script>
